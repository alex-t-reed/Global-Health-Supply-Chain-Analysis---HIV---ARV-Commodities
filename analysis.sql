...
-- Mode country counts?
SELECT shipment_mode, country, COUNT(*) AS mode_country_count
FROM supply_chain_pricing
WHERE shipment_mode IN ('Air', 'Truck', 'Air Charter', 'Ocean')
GROUP BY shipment_mode, country
ORDER BY shipment_mode, mode_country_count DESC;
...
SELECT product_group, COUNT(*) AS group_count
FROM supply_chain_pricing
...
SELECT product_group, ROUND(AVG(line_item_value), 2) AS avg_line_item_value
FROM supply_chain_pricing
GROUP BY product_group
...
SELECT shipment_mode, ROUND(AVG(weight_kilograms), 2) AS avg_weight
FROM supply_chain_pricing
...
SELECT shipment_mode, ROUND(AVG(freight_cost_usd),2) AS avg_freight_cost
FROM supply_chain_pricing
...
SELECT country, ROUND(SUM(line_item_value), 2) AS total_line_item_value, ROUND(SUM(freight_cost_usd), 2) AS total_freight_cost
FROM supply_chain_pricing
GROUP BY country
ORDER BY total_line_item_value DESC;
...
SELECT product_group, shipment_mode, ROUND(AVG(unit_price),2) AS avg_unit_price
FROM supply_chain_pricing
WHERE unit_price IS NOT NULL
GROUP BY product_group, shipment_mode
ORDER BY product_group, shipment_mode, avg_unit_price DESC;
...
SELECT product_group, dosage_form, COUNT(*) AS form_count
FROM supply_chain_pricing
WHERE dosage_form IS NOT NULL
GROUP BY product_group, dosage_form
...
FROM  supply_chain_pricing
WHERE  line_item_value IS NOT NULL AND freight_cost_usd IS NOT NULL
GROUP BY vendor
...
FROM supply_chain_pricing
WHERE brand IS NOT NULL AND molecule_test_type IS NOT NULL
GROUP BY brand, molecule_test_type
...
SELECT brand, molecule_test_type, vendor, COUNT(*) AS combination_count
FROM supply_chain_pricing
WHERE brand IS NOT NULL AND molecule_test_type IS NOT NULL AND vendor IS NOT NULL
GROUP BY brand, molecule_test_type, vendor
ORDER BY combination_count DESC
LIMIT 3;
...
SELECT item_description, SUM(line_item_quantity) AS total_quantity
FROM supply_chain_pricing
...
SELECT product_group, shipment_mode, COUNT(*) AS mode_count
FROM supply_chain_pricing
WHERE shipment_mode IS NOT NULL
GROUP BY product_group, shipment_mode
ORDER BY product_group, mode_count DESC;
-- ARV products are primarily shipped via Air with a substantial count of 4444 shipments, followed by Truck (2780 shipments) and Air Charter (650 shipments), while HRDT products are predominantly transported by Air with 1624 shipments, and ANTM and ACT product groups primarily utilize Air shipments (with 22 and 16 shipments, respectively), and MRDT products have a smaller shipment count, with just 7 shipments, all delivered by Air.
